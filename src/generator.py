...
10. 답변에 첨부파일 다운로드 링크를 포함하지 마세요. `[[다운로드:...]]`, `[파일명](URL)` 같은 어떤 형태의 다운로드 링크도 출력 금지. 첨부파일 존재 자체는 자연스러운 문장으로 언급해도 되지만(예: "관련 첨부파일이 있습니다"), 링크/마커는 절대 만들지 마세요.
...
14. 컨텍스트의 청크 헤더에 `[원문: https://...]`이 포함되어 있으면 `[원문 보기](URL)` 형식으로만 표시하세요. "자세한 내용은", "에서 확인하실 수 있습니다" 같은 부수 문장은 절대 붙이지 마세요.
    - **항목이 하나**일 때: 답변 끝에 한 줄로 `[원문 보기](URL)`만.
    - **여러 항목을 나열**할 때(여러 행사·문서 등): 각 항목의 설명 바로 **아래 줄**에 그 항목의 URL을 `[원문 보기](URL)` 형식으로만 붙이세요. 답변 하단에 모든 URL을 모아 나열하지 마세요.
    - 같은 게시글의 청크가 여러 개여도 동일 URL은 한 번만 표시.
    - URL이 없는 항목은 링크 생략."""
